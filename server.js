...
app.post('/add-medicine', (req, res) => {
  ...
  db.query(
    sql,
    [medicineName, batchNumber, expiryDate, returnDate, quantity, price],
    (err, result) => {
...
app.get('/medicines', (req, res) => {
  const sql = 'SELECT id, name, quantity, sold_qty FROM medicines WHERE quantity > sold_qty';
...
app.post('/sell-medicine', (req, res) => {
  ...
  db.query(sql, [soldQty, soldQty, medicineId, soldQty], (err, result) => {
    if (err) return res.status(500).json(err);
    if (result.affectedRows === 0)
      return res.status(400).json({ error: 'Insufficient quantity or invalid medicine ID' });
...
app.get('/report', (req, res) => {
  db.query('SELECT * FROM medicines', (err, results) => {
    ...
    const totalSold = results.reduce((s, m) => s + m.sold_qty, 0);
    const expired = results.filter(m => new Date(m.expiry_date) < new Date()).length;
    ...
    const salesReport = results.map(m => ({
      name: m.name,
      batchNumber: m.batch_number,   // ✅ FIX (THIS WAS MISSING)
      status: m.status,
      soldQty: m.sold_qty,
